...
const FE_Project = (props: any) => {
  ...
  // 获取项目列表数据
  useEffect(() => {
    props.BlogActions.asyncNavigationListAction(currentPage, pageSize, '', '项目列表').then((res: NavigationData) => {
      // 获取项目
  ...
  const onSearch = (value: string) => {
    props.BlogActions.asyncNavigationListAction(currentPage, pageSize, value, '项目列表').then((res: NavigationData) => {
      let { data, page, totalCount, pageSize } = res.data as unknown as NavigationData;
      setList(data);
      setTotal(totalCount);
      setCurrentPage(page);
  ...
  const onChangePage = (page: number, pageSize: number, params = '') => {
    // 重新调用接口将参数传递过去
    props.BlogActions.asyncNavigationListAction(page, pageSize, params, '项目列表').then((res: NavigationData) => {
      // 获取列表数据
  ...
  return (
    <div>
      ...
      <Modal
        open={isModalOpen}
        title={<div style={{ textAlign: 'left' }}>添加项目</div>}
        okText="添加"
        cancelText="取消"
      ...
      >
        <Form form={form} layout="vertical" name="basic" className="userAddFrom">
          <Form.Item name="title" label="项目名称" rules={[{ required: true, message: '项目名称不能为空' }]}>
